...
class Board:
    ...
    def getLastPieceMoved(self) -> Piece: 
        if self.history:
    ...
    def makeUnicodeStringRep(self, pieces: list[Piece]) -> str:
        ...
        for y in range(7, -1, -1):
            for x in range(8):
                ...
                if piece:
                    side = piece.side
                    fg_color = WHITE_FG if side == WHITE else BLACK_FG
                    pieceRep = fg_color + DISPLAY_LOOKUP[piece.stringRep] + ' ' + NO_FG
    ...
    def getCoordinateNotationOfMove(self, move: Move) -> str:
        ...
        if move.promotion:
            notation += str(
                move.specialMovePiece.stringRep 
            )
        return notation
    ...
    def pieceAtPosition(self, pos: C) -> Piece: 
        for piece in self.pieces:
